...
```{r}
file.path          <- "D:/Users/JorisS/Downloads/PortableGit/TcT/data/sample_data.csv"
...
matches.per.genome <- sapply(RNAseq.data$features$bins,
       function(bin){
         bin.annotations <- unique(RNAseq.data$table[which(RNAseq.data$table$Bin == bin), 'Annotation'])
...
rules <- apriori(sbs.trait.attributes,
              parameter = list(support    = 0.3,
                               confidence = 0.4,
                               minlen     = 3,
                               maxlen     = 4
                               ),
               appearance = list(lhs        = c()
                                 )
              )

Draw_Network(as(rules, 'data.frame'),annotation.db = RNAseq.data$features$annotation.db,
             N = 30)
```
