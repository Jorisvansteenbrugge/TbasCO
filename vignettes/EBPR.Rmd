...
**No normalization**
```{r, results='hide'}
Pre_process_input(file.path)
```
**With the default normalization method**
```{r, eval = F}
Pre_process_input(file.path, normalize.var = 'default')
```
**User defined method**
```{r, eval = F}
My_normalization_method <- function(RNAseq.table){
   #.......
  
  return(normalized.table)
}

Pre_process_input(file.path, normalize.var = My_normalization_method)

```


### Filtering
Removing rows without standard deviation
```
